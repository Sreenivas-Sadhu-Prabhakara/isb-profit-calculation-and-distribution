...
| POST | `/v1/profit-distribution-run-procedure/initiate` | Initiate |
| GET | `/v1/profit-distribution-run-procedure` | Retrieve (list) |
| GET | `/v1/profit-distribution-run-procedure/{crId}/retrieve` | Retrieve |
| PUT | `/v1/profit-distribution-run-procedure/{crId}/update` | Update |
| PUT | `/v1/profit-distribution-run-procedure/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/profit-distribution-run-procedure/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-profit-calculation-and-distribution:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-profit-calculation-and-distribution ./helm -n isb-deposits
```

Exposed through the platform Gateway at path prefix `/isb-profit-calculation-and-distribution` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
